...
import { Button, InjectedModalProps, Skeleton, Text } from '@bscindex/uikit'
...
import { useCheeseIndexTokenRabbits, useProfile as useProfileContract } from 'hooks/useContract'
import { getCheeseIndexTokenProfileAddress, getCheeseIndexTokenRabbitsAddress } from 'utils/addressHelpers'
...
const ChangeProfilePicPage: React.FC<ChangeProfilePicPageProps> = ({ onDismiss }) => {
  ...
  const {
    ...
    onApprove: () => {
      return cheeseIndexTokenRabbitsContract.methods.approve(getCheeseIndexTokenProfileAddress(), tokenId).send({ from: account })
    },
    onConfirm: () => {
      if (!profile.isActive) {
        return profileContract.methods.reactivateProfile(getCheeseIndexTokenRabbitsAddress(), tokenId).send({ from: account })
      }

      return profileContract.methods.updateProfile(getCheeseIndexTokenRabbitsAddress(), tokenId).send({ from: account })
    },
    onSuccess: async () => {
      // Re-fetch profile
    ...
    <>
      ...
      {!isLoading && walletNfts.length === 0 && (
        <>
          ...
          <Text as="p" color="textSubtle" mb="24px">
            {TranslateString(999, 'Make sure you have a CheeseIndexToken Collectible in your wallet and try again!')}
          </Text>
        </>
      )}
