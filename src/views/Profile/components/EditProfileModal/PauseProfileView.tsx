...
import { AutoRenewIcon, Button, Checkbox, Flex, InjectedModalProps, Text } from '@bscindex/uikit'
...
const PauseProfilePage: React.FC<PauseProfilePageProps> = ({ onDismiss }) => {
  ...
  const { numberCidToReactivate } = useGetProfileCosts()
  const TranslateString = useI18n()
  const cheeseIndexTokenProfileContract = useProfileContract()
  ...
  const handleDeactivateProfile = () => {
    cheeseIndexTokenProfileContract.methods
      .pauseProfile()
      .send({ from: account })
      .on('sending', () => {
  ...
  return (
    <>
      ...
      <Text as="p" color="textSubtle" mb="24px">
        {TranslateString(999, `Cost to reactivate in future: ${getBalanceNumber(numberCidToReactivate)} CID`)}
      </Text>
