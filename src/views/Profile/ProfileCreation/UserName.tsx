...
const minimumCidToRegister = new BigNumber(REGISTER_COST).multipliedBy(new BigNumber(10).pow(18))
...
const UserName: React.FC = () => {
  ...
  const { teamId, tokenId, userName, actions, minimumCidRequired, allowance } = useProfileCreation()
  ...
  const hasMinimumCidRequired = useHasCidBalance(minimumCidToRegister)
  const [onPresentConfirmProfileCreation] = useModal(
    <ConfirmProfileCreationModal
      ...
      account={account}
      teamId={teamId}
      minimumCidRequired={minimumCidRequired}
      allowance={allowance}
    />,
    false,
  )
    ...
    <>
      ...
      {!hasMinimumCidRequired && (
        <Text color="failure" mt="16px">
          {TranslateString(1098, `A minimum of ${REGISTER_COST} CID is required`, { num: REGISTER_COST })}
        </Text>
      )}
    </>
  )
