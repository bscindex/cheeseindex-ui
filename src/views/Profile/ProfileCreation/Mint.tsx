...
import { useCid, useBunnyFactory } from 'hooks/useContract'
import useHasCidBalance from 'hooks/useHasCidBalance'
...
const minimumCidBalanceToMint = new BigNumber(MINT_COST).multipliedBy(new BigNumber(10).pow(18))

const Mint: React.FC = () => {
  const [bunnyId, setBunnyId] = useState(null)
  const { actions, minimumCidRequired, allowance } = useProfileCreation()

  const { account } = useWallet()
  const cidContract = useCid()
  const bunnyFactoryContract = useBunnyFactory()
  const TranslateString = useI18n()
  const hasMinimumCidRequired = useHasCidBalance(minimumCidBalanceToMint)
  const {
    isApproving,
    isApproved,
    isConfirmed,
    ...
    onRequiresApproval: async () => {
      ...
      try {
        const response = await cidContract.methods.allowance(account, bunnyFactoryContract.options.address).call()
  ...
  return (
    <>
      ...
      <Text as="p" mb="24px">
        {TranslateString(790, 'You can change your profile pic later if you get another approved CheeseIndexToken Collectible.')}
      </Text>
      <Card mb="24px">
        <CardBody>
          ...
          <Text as="p" mb="24px" color="textSubtle">
            {TranslateString(999, `Cost: ${MINT_COST} CID`, { num: MINT_COST })}
          </Text>
          {nfts.map((nft) => {
            ...
            return (
              <SelectionCard
                ...
                disabled={isApproving || isConfirming || isConfirmed || !hasMinimumCidRequired}
              >
                <Text bold>{nft.name}</Text>
              </SelectionCard>
            )
          })}
          {!hasMinimumCidRequired && (
            <Text color="failure" mb="16px">
              {TranslateString(1098, `A minimum of ${MINT_COST} CID is required`)}
            </Text>
          )}
          <ApproveConfirmButtons
            isApproveDisabled={bunnyId === null || isConfirmed || isConfirming || isApproved}
            isApproving={isApproving}
            isConfirmDisabled={!isApproved || isConfirmed || !hasMinimumCidRequired}
